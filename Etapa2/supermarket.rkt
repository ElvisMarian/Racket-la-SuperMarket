...
(define (add-to-counter name n-items)
 ...
 (if (empty? queue) 
              (make-counter index (+ tt n-items) (+ tt n-items) (reverse (cons (cons name n-items) (reverse queue))))
...
;(define C1 (make-counter 1 8 8 '((ionel . 100))))
;(define C2 (make-counter 2 55 55 '((ionut . 55))))
;(define C3 (make-counter 3 4 4 '((ion . 99))))
;(define C4 (make-counter 4 10 10 '((xos . 15))))
;(define C5 (make-counter 5 100 100 '((con . 11))))
;(define C6 (make-counter 6 11 48 '((SSS . 111))))

 

;(update (add-to-counter 'cory 50) (list C1 C2 C3 C4 C5 C6) (car (min-tt (list C1 C2 C3 C4 C5 C6))))
       ...
       [(list 'delay index minutes)
        ...
        [(list 'remove-first)
         '(if( > (cdr (min-et fast-counters)) (cdr (min-et slow-counters)))
               (serve (cdr requests) fast-counters (update remove-first-from-counter  slow-counters  (car (min-et (filter (lambda(e) (positive? (counter-et e))) slow-counters)))))
               (serve (cdr requests) (update remove-first-from-counter fast-counters  (car (min-et  (filter (lambda(e) (positive? (counter-et e))) fast-counters)))))
               )
         ]
