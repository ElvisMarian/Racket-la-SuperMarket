...
(define (queue-empty? q)
  (if (and (empty? ( queue-left q)) (empty? (queue-right q))) #t #f)
  )
...
(define (enqueue x q)
   (make-queue (queue-left q) (cons x (queue-right q)) (queue-size-l q) (+ (queue-size-r q) 1))
  )
...
(define (dequeue q)
  (if (empty? (queue-left q))
      (if(empty? (queue-right q)) empty-queue
      (make-queue  (cdr (reverse (queue-right q))) '() (- (queue-size-r q) 1) 0 ))
